...
def test_finite_fault_integration_suite(finite_fault_setup):
    ...
    center = np.mean(fault_mesh.vertices, axis=0)
    ...
    configs = [
        ...
        FiniteFault(center=center, strike_radius=0.7, dip_radius=0.7, taper=TaperType.SPLINE)
    ]
    ...
    for i, ff in enumerate(configs):
        ...
        slip_mesh = ff.calculate_slip(mesh_vertices, normal)
