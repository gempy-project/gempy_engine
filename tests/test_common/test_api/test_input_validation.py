...
def test_surface_without_points(basic_input):
    ii, options, dd = basic_input
    # surface with 0 points
    new_tensor_struct = TensorsStructure(number_of_points_per_surface=np.array([0, 10]))
    new_stack_struct = StacksStructure(
        number_of_points_per_stack=np.array([10]),
        number_of_orientations_per_stack=np.array([2]),
    ...
    with pytest.raises(GemPyEngineInputError, match="Validation Error: Surface 0 has no surface points"):
        compute_model(ii, options, dd)


def test_stack_without_points(basic_input):
    ii, options, dd = basic_input
    # stack with 0 points
    new_tensor_struct = TensorsStructure(number_of_points_per_surface=np.array([10, 0]))
    new_stack_struct = StacksStructure(
        number_of_points_per_stack=np.array([10, 0]),
        number_of_orientations_per_stack=np.array([2, 1]),
        number_of_surfaces_per_stack=np.array([1, 1]),
        masking_descriptor=[StackRelationType.ERODE, StackRelationType.ERODE]
    )
    # We need to make sure ii matches the total counts to reach this validation
    ii.surface_points = SurfacePoints(sp_coords=np.random.rand(10, 3))
    ii.orientations = Orientations(dip_positions=np.random.rand(3, 3), dip_gradients=np.random.rand(3, 3))

    dd = InputDataDescriptor(tensors_structure=new_tensor_struct, stack_structure=new_stack_struct)

    with pytest.raises(GemPyEngineInputError, match="Validation Error: Stack 1 has no surface points"):
        compute_model(ii, options, dd)


def test_stack_without_orientations(basic_input):
    ii, options, dd = basic_input
    # stack with 0 orientations
    new_tensor_struct = TensorsStructure(number_of_points_per_surface=np.array([10, 5]))
    new_stack_struct = StacksStructure(
        number_of_points_per_stack=np.array([10, 5]),
        number_of_orientations_per_stack=np.array([2, 0]),
        number_of_surfaces_per_stack=np.array([1, 1]),
        masking_descriptor=[StackRelationType.ERODE, StackRelationType.ERODE]
    )
    # We need to make sure ii matches the total counts
    ii.surface_points = SurfacePoints(sp_coords=np.random.rand(15, 3))
    ii.orientations = Orientations(dip_positions=np.random.rand(2, 3), dip_gradients=np.random.rand(2, 3))

    dd = InputDataDescriptor(tensors_structure=new_tensor_struct, stack_structure=new_stack_struct)

    with pytest.raises(GemPyEngineInputError, match="Validation Error: Stack 1 has no orientations"):
        compute_model(ii, options, dd)
