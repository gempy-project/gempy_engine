...
def interpolate_single_scalar(surface_points: data.SurfacePoints, orientations: data.Orientations, grid: np.ndarray,
                              options: data.InterpolationOptions, data_shape: data.TensorsStructure):
    sp_internal = data_preprocess_interface.prepare_surface_points(surface_points, data_shape.number_of_points_per_surface)
...
def _evaluate_sys_eq(grid, options, ori_internal, sp_internal, weights) -> exported_structs.ExportedFields:
    ...
    eval_gx_kernel = kernel_constructor_interface.yield_evaluation_grad_kernel(grid, sp_internal, ori_internal, options, axis=0)
    eval_gy_kernel = kernel_constructor_interface.yield_evaluation_grad_kernel(grid, sp_internal, ori_internal, options, axis=1)

    scalar_field = weights @ eval_kernel
    gx_field = weights @ eval_gx_kernel
    gy_field = weights @ eval_gy_kernel
    if options.number_dimensions == 3:
        eval_gz_kernel = kernel_constructor_interface.yield_evaluation_grad_kernel(grid, sp_internal, ori_internal, options, axis=2)
