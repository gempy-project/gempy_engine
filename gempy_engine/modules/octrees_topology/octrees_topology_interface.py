...
def get_regular_grid_for_level(octree_list: List[OctreeLevel], level: int):
    ...
    shaped_ids_block = tfnp.reshape(root.output_centers.ids_block, root.grid_centers.regular_grid_shape)#.reshape(root.grid_centers.regular_grid_shape)
    regular_grid = _expand_regular_grid(shaped_ids_block, level)
    ...
    for e, octree in enumerate(octree_list[1:level + 1]):
        ...
        ids = _expand_octree(tfnp.reshape(octree.output_centers.ids_block, (-1, 2, 2, 2)), n_rep - 1)
