...
def create_covariance(ki: KernelInput, options: InterpolationOptions, item=None) -> tensor_types:
...
def _compute_all_distance_matrices(cs, ori_sp_matrices) -> InternalDistancesMatrices:
    ...
    if BackendTensor.pykeops_enabled is True:
        if True:
            ...
            r_ref_rest = ori_sp_matrices.dip_ref_i.sqdist(
                ori_sp_matrices.diprest_j)  # (((ori_sp_matrices.dip_ref_i - ori_sp_matrices.diprest_j) ** 2).sum(-1)).sqrt()
            r_rest_ref = ori_sp_matrices.diprest_j.sqdist(
                ori_sp_matrices.dip_ref_j)  # (((ori_sp_matrices.diprest_i - ori_sp_matrices.dip_ref_j) ** 2).sum(-1)).sqrt()
